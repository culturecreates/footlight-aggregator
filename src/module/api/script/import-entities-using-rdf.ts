...
@Command({ name: "import:rdf", description: "Import entities to footlight-calendar using RDF" })
export class ImportRdf extends CommandRunner {
  constructor(
  ...
  async run(
    ...
    try {
      ...
      if (authenticationResponse?.accessToken) {
        ...
        await this._eventService.syncEntitiesUsingRdf(authenticationResponse.accessToken, options?.rdfFile, options?.mappingFile, options?.footlightBaseUrl, options?.calendar);
      } else {
  ...
  @Option({
    flags: "-r, --rdf-file [string]",
    description: "Rdf source file path",
    required: true
  })
  parseRdfFile(val: string): string {
    return val;
  }
